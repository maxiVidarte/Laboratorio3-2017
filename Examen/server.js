	...
	request.on('error', function (err) {
		...
		if (request.url.includes('getPost')) {
			...
			if (body != "") {
				...
				if(requestBodyObject.title && requestBodyObject.header && requestBodyObject.posttext && requestBodyObject.author)
					requestBodyObject.date = new Date().toLocaleString();
				else
					requestBodyObject.error = "Falta parametros en JSON"
				}catch(e){
					
					console.log(""+e);
					requestBodyObject = {error:""+e};
				}
			}
